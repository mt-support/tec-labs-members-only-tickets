...
* `members-only-tickets` - Plugin Human-readable name, e.g. "Sample Extension"
* `Membersonlytickets` - Which namespace we will use for the plugin. Usually the plugin name or the slug in uppercase without spaces, e.g. "SampleExtension"
* `members-only-tickets` - Uses using dashes normally, e.g. "sample-extension"
* `__TRIBE_DOMAIN__` - Translation domain normally with "tribe-" prefix, e.g. "tec-labs-sample-extension"
* `members_only_tickets` - Uses underscores, so it's safe for variables, e.g. "sample_extension"
